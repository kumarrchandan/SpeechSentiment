...
def text_kerasAnalyze(text_columns):
    ...
    with tf.Session() as session:
        ...
        loaded_model = load_model(weight_path)
        text_columns = len(re.findall(r'\w+', str(text_columns))) 
        if(text_columns>56):
            text = text_columns
            formatted = wrap(text,56)
            tot_score=0.00
            for k in range (0,len(formatted)):
                tot_score = tot_score+(live_test(loaded_model,str(formatted[k]),word_idx))
            chunks = len(formatted)
            avg_score = (tot_score/chunks)    
            avg_score=round(avg_score,2)
            all_sent_values.append(avg_score)
        else:
            all_sent_values.append(live_test(loaded_model,str(text_columns),word_idx))
        session.close()
    return all_sent_values
...
print(keras_analyze("Great!! it is raining today!!"))